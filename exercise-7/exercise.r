...
team.data$Team[which.min(team.data$TOV.STL.ratio)]
...
team.data <- mutate(team.data, AST.G = AST / G) %>%
  arrange(-AST.G)
...
defense.stats <- select(team.data, Team, DRB, STL)
...
BetterShooters <- function (team.1, team.2){
  compare.teams <- filter(team.data, Team %in% c(team.1, team.2)) %>%
    filter(FG. == max(FG.)) %>%
    select(Team, FG., PTS)
